...
- **Worktree isolation per lead** keeps agents off the human's primary checkout (no IDE collisions) and off each other's files. **But auto-isolation can SILENTLY FAIL** — a lead may land on the primary checkout instead of a worktree (observed: 3 of 4 leads isolated, the 4th didn't, twice). Defenses:
  ...
  - Step 1 also **verifies the commit identity git will actually use**: `git var GIT_AUTHOR_IDENT`, not `git config user.email`. An env var or a per-repo override beats the global config, so a correct global value proves nothing. If it is not the estate's author address, STOP. Fix it where it is set; never patch it per commit. Commits under a stray address are already pushed by the time anyone notices.
