...
| `validate-backlog.sh` | PostToolUse·Edit\|Write | On a `scrum/<space>/backlog.json` edit: hard-fails only on invalid JSON; flags duplicate/unknown `id`, non-canonical status, and unresolved `<SPACE>-` refs as advisories to reconcile via `sync.py`. Accepts `NO GO`. | — (advisory) |
